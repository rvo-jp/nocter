...
`parse` follows the ordinary abort-on-allocation-exhaustion policy. `try_parse`, `try_to_string`, and
`try_request_target` use caller-provided recoverable allocation; `try_resolve` does the same for
relative resolution. Syntax failures use stable `std.url.*` codes and are never relabeled as
allocation failures. Raw non-ASCII URL components must be percent-encoded.
