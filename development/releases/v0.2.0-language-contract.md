# Archived Nocter v0.2.0 Language Contract

This compiler-development archive preserves the mixed language/backend contract used to qualify
v0.2.0. It is not part of the current specification. The `v0.2.0` repository tag remains the exact
authority for that release.

This chapter records the implemented language contract released as Nocter
v0.2.0. It is narrower than the long-term language direction. A feature that is
not listed here is not part of the v0.2.0 release even if a later design chapter
mentions it.

## Interfaces And Excluded Features

Nocter v0.2.0 includes contract-only `interface` declarations and explicit
...
`trait` is not a reserved keyword in v0.2.0. It is lexed as an identifier. Source
...
single top-level declaration that immediately follows it. In v0.2.0 that following
...
The v0.2.0 parser also accepts the non-`pub` `use` forms above at the start of any
...
The v0.2.0 parser rejects these forms with diagnostics instead of accepting them as
...
The resolver owns name binding for v0.2.0. Later stages, CLI diagnostics, and LSP
...
Trait symbols are not part of the v0.2.0 source-level contract. v0.2.0 source can
...
If a source construct is not represented in the v0.2.0 typed facts, it must either
be added to this contract or rejected before lowering.

## Buildability Contract

`build` and `run` perform a v0.2.0 buildability validation step after typechecking
...
`check` but not yet supported by v0.2.0 runtime lowering.
...
- payloadless enum values use the explicit `u8` tag layout specified by ABI v0.2.0
