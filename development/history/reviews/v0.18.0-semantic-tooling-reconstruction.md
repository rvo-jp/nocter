# v0.18.0 Semantic Tooling Reconstruction Record

This document records the completed v0.18.0 semantic-tooling reconstruction and its migration gate.
Current crate-local contracts belong to the `nocter-session`, `nocter-analysis`,
`nocter-workspace-analysis`, and `nocter-language-server` READMEs; their shared presentation
boundary belongs to [Semantic Presentation Design](../../architecture/pipeline/semantic-presentation.md). This record
does not change the Nocter language or runtime contract.
