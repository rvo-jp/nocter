...
Connect the production plain-connection constructor and callable terminal lifecycle to primitive
calls over the completed `NetworkOwner` runtime-storage role, then implement event receipt,
receive, send, and local/remote address operations. Migrate the public plain TCP stream only
