...
/// Extracts exact top-level include paths without traversing item or block syntax.
pub(crate) fn include_paths(
    ...
    for element in tree.children(tree.root_id()) {
        ...
        let path = direct_child(tree, *declaration, NodeKind::IncludePath)
...
pub(crate) fn include_path_node(tree: &SyntaxTree, declaration: NodeId) -> Option<NodeId> {
    direct_child(tree, declaration, NodeKind::IncludePath)
}
