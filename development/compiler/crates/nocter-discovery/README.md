...
it does not know whether parsing was direct or reused. Package-root syntax already owned by the
resolved graph is retained directly rather than requested again. It also publishes one canonical
semantic topology surface containing only discovery decisions that can affect declaration
semantics. It does not lower declarations or interpret body semantics.
...
- A package root that becomes a module source keeps the graph's sole exact syntax tree. Discovery
  rejects a missing or duplicate retained tree and never reparses that source.
- A newly ingested module source requests the source-file parse goal exactly once through the
  supplied provider.
...
- The exact current-source surface uses the same canonical inventory and includes every reached
  source byte and complete source identity. It is a process-local generation invalidation product,
  never public serialization or declaration semantics.
