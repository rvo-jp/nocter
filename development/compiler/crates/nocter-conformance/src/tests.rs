...
use nocter_runtime_contract::{PrimitiveRegistry, RuntimeStorageRegistry, TargetServiceRegistry};
...
fn deferred_process_waits_for_process_completion_without_polling() {
    ...
    let fixture = CompilerFixture::with_app_standard_uses(
        &format!(
            "use std/internal/task\n\
             async func main(): i32 {{\n\
                 await task.process_completion_for_test({process})\n\
                 return 47\n\
             }}\n"
        ),
        &[&["internal", "task"]],
...
fn lower_fixture(fixture: &CompilerFixture, tests: bool) -> MachineProgram {
    ...
    let target_services =
        TargetServiceRegistry::new(lowered.target_service_bindings().iter().cloned()).unwrap();
    let runtime_storage =
