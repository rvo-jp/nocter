...
fn is_eligible_contract(
    ...
    if declaration.kind() != SurfaceDeclarationKind::ConformanceMethod {
        return Ok(false);
    }
    Ok(declaration.owner().is_some_and(|owner| {
        surface
            .declarations()
            .get(owner.index())
            .is_some_and(|owner| owner.kind() == SurfaceDeclarationKind::Conformance)
    }))
}
