...
fn binds_qualified_generic_and_associated_type_shapes_before_normalization() {
    ...
    let app_id = add_source(
        &mut sources,
        "/app/index.nct",
        "use dep\ntype Projection<T> = future &dep.Buffer<T>.Item?!\n",
    );
    ...
    let BoundTypeKind::Future(fallible) = bound.kind(root).unwrap() else {
        panic!("expected outer future type");
    };
...
fn normalizes_explicit_callable_origins_to_parameter_positions() {
    ...
    let app_id = add_source(
        &mut sources,
        "/app/index.nct",
        "type Callback<T> = noalloc blocking &+func(left: &T from static | right, right: &T): &T from right | left\n",
    );
    let std_root_id = add_source(
        &mut sources,
        "/std/index.nct",
    ...
    assert_eq!(
        callable
            .input_provenance()
            .sources(ParameterOrigin::new(0))
            .unwrap()
            .origins(),
        &[ParameterOrigin::new(1)]
    );
    assert_eq!(
