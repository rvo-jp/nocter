...
fn namespace_member_call_projects_the_callable_for_hover_and_navigation() {
    ...
    let source_text = concat!(
        "use std/fs\n",
        "async func inspect(path: &str): void! {\n",
        "    let details = await fs.metadata(path)?\n",
        "    let _ = details.len()\n",
        "    return\n",
        "}\n",
...
fn rename_validation_rejects_a_candidate_without_checked_semantics() {
    ...
    assert!(!candidate.seals_semantic_mutation().unwrap());
    let mutation = original.semantic_rename_candidate(plan).unwrap();
    assert!(mutation.validate(Box::new(candidate)).unwrap().is_none());
}
...
fn service_scope_contract_composes_admission_cancellation_and_joining() {
    ...
    let (_, snapshot) = bundled_snapshot(
        ...
        concat!(
            ...
            "    return\n",
            "}\n",
            "async func observe_lifecycle(): void! {\n",
            "    let _ = await service.lifecycle_requested()?\n",
            "    return\n",
            "}\n",
