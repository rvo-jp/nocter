...
fn invalid_initialize_can_retry_before_document_generations_begin() {
    ...
    let invalid = server.receive(r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}"#);
    assert!(invalid.response().unwrap().contains("\"code\":-32602"));

    let initialized = server
        .receive(r#"{"jsonrpc":"2.0","id":2,"method":"initialize","params":{"capabilities":{}}}"#);
    ...
    let unknown =
        server.receive(r#"{"jsonrpc":"2.0","id":"x","method":"textDocument/unknown","params":{}}"#);
...
fn malformed_document_notification_is_reported_without_a_protocol_response() {
    ...
    server
        .receive(r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"capabilities":{}}}"#);
    server.receive(r#"{"jsonrpc":"2.0","method":"initialized"}"#);
    let step = server.receive(r#"{"jsonrpc":"2.0","method":"textDocument/didChange","params":{}}"#);
...
fn type_hover_is_independent_of_the_construction_surface() {
    ...
    let response = hover.response().unwrap();
    assert!(
        response.contains("pub struct Widget<T> {\\n    pub visible: T\\n    pub hidden: T\\n}"),
        "{response}"
    );
...
fn semantic_tokens_use_exact_compiler_bindings_instead_of_syntax_ranges() {
    ...
    assert_eq!(
        tokens.response(),
        Some(r#"{"jsonrpc":"2.0","id":2,"result":{"resultId":"1","data":[0,6,4,10,1,0,9,4,1,0]}}"#)
    );
...
fn assert_metadata_time_hover(server: &mut LanguageServer, source_uri: &str, source_text: &str) {
...
fn assert_symlink_metadata_hover(server: &mut LanguageServer, source_uri: &str, source_text: &str) {
...
fn assert_canonicalize_hover(server: &mut LanguageServer, source_uri: &str, source_text: &str) {
    let line = source_text
        .lines()
        .position(|line| line.contains("canonicalize_blocking"))
        .unwrap();
    let character = source_text
        .lines()
        .nth(line)
        .unwrap()
        .find("canonicalize_blocking")
        .unwrap();
    let hover = server.receive(&format!(
        "{{\"jsonrpc\":\"2.0\",\"id\":36,\"method\":\"textDocument/hover\",\"params\":{{\"textDocument\":{{\"uri\":\"{source_uri}\"}},\"position\":{{\"line\":{line},\"character\":{character}}}}}}}"
    ));
    let response = hover.response().unwrap();
    assert!(
        response.contains(
            "```nocter\\npub blocking func canonicalize_blocking(path: &str): Utf8Path!\\n```"
        ),
        "{response}"
    );
    assert!(hover.issue().is_none(), "{:?}", hover.issue());
...
fn assert_copy_hover(server: &mut LanguageServer, source_uri: &str, source_text: &str) {
    let line = source_text
        .lines()
        .position(|line| line.contains("copy_blocking"))
        .unwrap();
    let character = source_text
        .lines()
        .nth(line)
        ...
        .unwrap();
    let hover = server.receive(&format!(
        "{{\"jsonrpc\":\"2.0\",\"id\":37,\"method\":\"textDocument/hover\",\"params\":{{\"textDocument\":{{\"uri\":\"{source_uri}\"}},\"position\":{{\"line\":{line},\"character\":{character}}}}}}}"
    ));
    let response = hover.response().unwrap();
    assert!(
        response.contains(
            "```nocter\\npub blocking func copy_blocking(from: &str, to: &str): usize!\\n```"
        ),
        "{response}"
    );
    assert!(hover.issue().is_none(), "{:?}", hover.issue());
...
fn assert_recursive_traversal_hover(
    server: &mut LanguageServer,
    source_uri: &str,
    source_text: &str,
) {
    let line = source_text
        .lines()
        .position(|line| line.contains("walk_dir_blocking"))
        .unwrap();
    let character = source_text
        .lines()
        .nth(line)
        .unwrap()
        .find("walk_dir_blocking")
        .unwrap();
    let hover = server.receive(&format!(
        "{{\"jsonrpc\":\"2.0\",\"id\":39,\"method\":\"textDocument/hover\",\"params\":{{\"textDocument\":{{\"uri\":\"{source_uri}\"}},\"position\":{{\"line\":{line},\"character\":{character}}}}}}}"
    ));
    let response = hover.response().unwrap();
    assert!(
        response.contains(
            "```nocter\\npub blocking func walk_dir_blocking(path: &str): BlockingWalkDir!\\n```"
        ),
        "{response}"
    );
    assert!(hover.issue().is_none(), "{:?}", hover.issue());
    ...
    let line = source_text
        .lines()
        .position(|line| line.contains("walk.next_blocking"))
        .unwrap();
    let character = source_text
        .lines()
        .nth(line)
        ...
        .unwrap();
    let hover = server.receive(&format!(
        "{{\"jsonrpc\":\"2.0\",\"id\":40,\"method\":\"textDocument/hover\",\"params\":{{\"textDocument\":{{\"uri\":\"{source_uri}\"}},\"position\":{{\"line\":{line},\"character\":{character}}}}}}}"
    ));
    let response = hover.response().unwrap();
    assert!(
        response.contains(
            "```nocter\\npub blocking method &+BlockingWalkDir.next_blocking(): DirEntry?!\\n```"
        ),
        "{response}"
    );
    assert!(hover.issue().is_none(), "{:?}", hover.issue());
...
fn path_and_directory_mutation_contracts_share_complete_editor_semantics() {
    ...
    assert_recursive_traversal_hover(&mut server, &source_uri, source_text);
