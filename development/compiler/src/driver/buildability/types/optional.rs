...
pub(in crate::driver::buildability) fn type_expr_is_top_level_fallible_with_resolver<'a, F>(
    ty: &TypeExpr,
    fallback_resolved: &'a ResolveOutput,
    resolver: &F,
) -> bool
where
    F: Fn(SourceId) -> Option<&'a ResolveOutput>,
{
    type_expr_is_top_level_fallible_inner(ty, fallback_resolved, resolver, &mut HashSet::new())
}
...
pub(in crate::driver::buildability) fn type_expr_is_top_level_fallible_inner<'a, F>(
    ty: &TypeExpr,
    fallback_resolved: &'a ResolveOutput,
    resolver: &F,
    resolving_names: &mut HashSet<String>,
    ...
    F: Fn(SourceId) -> Option<&'a ResolveOutput>,
{
    match ty {
        ...
        TypeExpr::Reference(reference) => {
            ...
            let result = type_expr_is_top_level_fallible_inner(
                target,
                fallback_resolved,
                resolver,
                resolving_names,
