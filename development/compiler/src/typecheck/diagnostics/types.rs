...
pub(in crate::typecheck) fn unsized_value_type_diagnostic(
    ...
    diagnostic.help = Some(match unsized_part {
        ...
        Type::Named(_) | Type::Generic { .. } => {
            "use a concrete struct or enum that explicitly conforms to the interface; interfaces are contracts only and have no runtime dispatch representation".to_string()
        }
