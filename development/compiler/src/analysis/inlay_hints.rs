...
use super::provenance::result_provenance_markdown;
...
pub(crate) fn inlay_hints_for_file_analysis(
    ...
    for (declaration, fact) in analysis.callable_semantic_facts.entries() {
        if declaration.source != file.ast.span.source || !requested.contains(&declaration.end) {
            continue;
        }
        ...
        if !declaration_has_explicit_provenance(text, declaration)
            && let Some(tooltip) = result_provenance_markdown(sources, analysis, declaration)
        {
            hints.push(InlayHintInfo {
                offset: declaration.end,
...
fn declaration_has_explicit_provenance(text: &str, name_span: ByteSpan) -> bool {
    let Some(tail) = text.get(name_span.end..) else {
        return false;
    };
    let end = tail.find(['{', '\n']).unwrap_or(tail.len());
    tail[..end].contains(" from ")
}
...
"#;
        ...
        assert!(hints.iter().any(|hint| {
            hint.offset == text.find("build").unwrap() + "build".len() && hint.label == " allocates"
        }));
        assert!(hints.iter().any(|hint| {
            hint.offset == text.find("label").unwrap() + "label".len()
