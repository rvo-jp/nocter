...
- `nocter-machine` owns target-independent stored layout and the remaining machine-program
  ...
  function, its callable ABI, and a checked receiver offset into the transferred iterator state;
  the caller-side borrow value does not survive the machine boundary. Residual cleanup is frozen
