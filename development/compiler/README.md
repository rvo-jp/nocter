...
- `nocter-package` owns the structured interpretation of `nocter.nct`: presentation metadata,
  ...
  origins. Its `ResolvedPackageGraph` also owns the loaded manifest source/syntax universe and
  validates exact alias, lock, and canonical path edges before discovery. Later stages consume
  these facts and cannot reopen or reinterpret directive text.
...
- `nocter-target-program` owns the selected-target and executable boundaries. `TargetProgram`
  ...
  types, cleanup-specific glue, and one call-site plan for every ordinary or literal argument pack.
  A shared pack plan binds its specialized signature to the fixed/spread producer order,
  concrete iterator/item/contribution types, already selected iteration operations, and allocation
  selection without creating ordinary variadic inputs. Sequence plans additionally bind their
  dense literal item, result, and allocation selection. Fixed values and spread iterators retain
...
- `nocter-mir` owns the backend-independent control-flow representation and its consuming builders.
  ...
  Lexical regions use paired typed creation and release operations over non-movable locals; no
  region handle passes through SSA. Ordinary calls, literal calls, and authored destruction retain
  the exact active region selection. A CFG stack validator proves creation, inner-first release,
  merge agreement, and normal-terminal cleanup, while the existing cleanup schedule orders body
  destruction before release on fallthrough and early transfer. Closure construction,
...
- `nocter-machine` owns target-independent stored layout and the remaining machine-program
  ...
  caller-owned result transport, and separate argument-pack pointer lane. A spill closes the
  ...
  address, SSA-value, operation, argument-pack, block, linkage, and data identities. Layout-owned byte offsets, checked
  ...
  spread segments through a dedicated body-local identity. Every spread freezes a direct next
  function and a checked receiver offset into the transferred iterator state; that function's
  callable ABI remains the unique transport authority, and the caller-side borrow value does not
  survive the machine boundary. Residual cleanup is frozen
  into machine-function targets, tags, strides, and byte offsets. Compiler-provided comparison, checked
  ...
  one immutable dataflow table. It expands values hidden inside addresses and argument packs once,
  ...
  A content-ordered `MachineDestructionTable` now interns every nonempty pointer and argument-pack
...
- `nocter-arm64` owns physical ARM64 register roles and instruction encoding without depending on
  ...
  through ordinary generated functions. Spread callbacks invoke their direct iterator function
  through that function's ordinary ABI, support direct and copied-borrow contributions, stage both
  direct and caller-owned optional results, skip empty spreads, and destroy iterator state exactly
  once. An iterator that contradicts its frozen exact length terminates with a distinct target trap.
  ...
  saves incoming context pointers, and materializes inherited, lexical-region, or explicit
  selections through `x9`; current allocator state/kind primitives expand through their ordinary
  ABI plans. A lexical region receives one compiler-owned five-word frame resource containing its
  ordinary header, independent allocation-list head, and retained parent header. Creation derives
  that header from the checked parent, and release advances then unmaps every owned mapping through
  a checked Darwin syscall loop. Pure
