...
use std/string.SplitIter

func first_part(text: &str): &str! {
    var parts: SplitIter = text.split_views(",")?
...
func main(): i32 {
    let range: &str = "hello".get_range(1, 4) otherwise { return 1 }
    let prefix: &str = "hello".strip_prefix("he") otherwise { return 2 }
    let suffix: &str = "hello".strip_suffix("lo") otherwise { return 3 }
    return 0
}
"#,
    );
...
fn distributed_std_borrowed_text_ranges_and_iterators_run() {
    ...
    let source = project.write_source(
        ...
        r#"func main(): i32! {
    let utf8: &str = "aé日😀z"
    if !utf8.is_char_boundary(0) || !utf8.is_char_boundary(1) { return 1 }
    if utf8.is_char_boundary(2) || !utf8.is_char_boundary(3) { return 2 }
    if utf8.is_char_boundary(4) || utf8.is_char_boundary(5) || !utf8.is_char_boundary(6) { return 3 }
    if utf8.is_char_boundary(7) || utf8.is_char_boundary(8) || utf8.is_char_boundary(9) || !utf8.is_char_boundary(10) { return 4 }
    if !utf8.is_char_boundary(11) || utf8.is_char_boundary(12) { return 5 }

    let full = utf8.get_range(0, 11) otherwise { return 6 }
    let middle = utf8.get_range(1, 10) otherwise { return 7 }
    let empty = utf8.get_range(3, 3) otherwise { return 8 }
    if full != utf8 || middle != "é日😀" || empty != "" { return 9 }
    let invalid_boundary = utf8.get_range(2, 3)
    let reversed = utf8.get_range(3, 2)
    let out_of_bounds = utf8.get_range(0, 12)
    if has_value(invalid_boundary) || has_value(reversed) || has_value(out_of_bounds) { return 10 }

    let without_prefix = "prefix-value".strip_prefix("prefix-") otherwise { return 11 }
    let without_suffix = "value-suffix".strip_suffix("-suffix") otherwise { return 12 }
    if without_prefix != "value" || without_suffix != "value" { return 13 }
    let missing_prefix = "value".strip_prefix("x")
    let missing_suffix = "value".strip_suffix("x")
    if has_value(missing_prefix) || has_value(missing_suffix) { return 14 }
    if "value".strip_prefix("")! != "value" || "value".strip_suffix("")! != "value" { return 15 }

    var parts = ",a,,é,😀,".split_views(",")?
...
func rejects_empty_separator(): bool {
    let parts = "value".split_views("") catch error { return true }
    return false
}
"#,
    );
...
fn distributed_std_borrowed_text_iterators_do_not_reach_the_allocator() {
    ...
    let source = project.write_source(
        ...
        r#"func main(): i32! {
    var parts = "zero,one,two,three".split_views(",")?.skip(1).take(2)
...
fn distributed_std_borrowed_text_view_keeps_owned_source_loan_active() {
    ...
    let view = text.get_range(1, 4) otherwise { return 0 }
    text.push_str("!")
    return view.len()
}
...
fn distributed_std_borrowed_text_iterators_keep_both_inputs_active() {
    ...
    let source = project.write_source(
        ...
        r#"func mutate_text(): usize! {
    var text = String "a,b"
    var parts = text.split_views(",")?
    text.push_str(",c")
...
func mutate_separator(): usize! {
    ...
    var parts = "a,b".split_views(&separator as &str)?
    separator.push_str(";")
    return parts.next()!.len()
}
...
func static_range(): &str {
    return "hello".get_range(1, 4) otherwise { return "" }
}
...
func temporary_prefix(text: &str): &str {
    ...
    region temporary using arena {
        ...
        return text.strip_prefix(&prefix as &str) otherwise { return text }
    }
}
...
fn distributed_lsp_presents_borrowed_text_views_from_the_public_facade() {
    ...
    let view = "hello".get_range(1, 4) otherwise { return 1 }
    var parts = "a,b".split_views(",")?
...
"#;
    ...
    assert!(
        range_hover.contains("method &str.get_range(start: usize, end: usize): &str?"),
        "{range_hover}"
    );
    ...
    assert!(
        split_hover
            .contains("method &str.split_views(separator: &str): SplitIter! from self | separator"),
        "{split_hover}"
    );
    ...
    assert_eq!(
        signature,
        "method &str.get_range(start: usize, end: usize): &str?"
    );
    ...
    assert!(
        target_uri.is_some_and(|uri| uri.ends_with("/std/str/index.nct")),
...
func leak(): &str {
    ...
    region temporary using arena {
        let text = String "hello"
        return text.get_range(1, 4) otherwise { return "" }
    }
}
