...
#[test]
fn distributed_std_coerced_slice_first_preserves_the_source_loan() {
    let project = TempProject::new("distributed-home-coerced-slice-first-loan");
...
func invalid(): i32 {
    ...
    let cell = values.first() otherwise { return 0 }
...
#[test]
fn distributed_std_coerced_slice_first_runs_from_packaged_home() {
    let project = TempProject::new("distributed-home-coerced-slice-first-run");
    let source = project.write_source(
...
func main(): i32 {
    ...
    let cell = values.first() otherwise { return 1 }
    return read(cell)
}
"#,
    );
