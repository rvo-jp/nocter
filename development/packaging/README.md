...
- `package-local-release.sh` builds the optimized compiler, copies only tracked standard-library
  files, normalizes archive metadata, and atomically writes one host archive.
...
and LSP initialization and shutdown. It also proves that these commands do not mutate the
installed home. Only after every check passes does it replace the generated candidate outputs in
`dist/`.
