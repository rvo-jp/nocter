...
A process API must compose child completion and streaming pipes with the executor while preserving
one exact observation and cleanup obligation. Wrapping blocking `waitpid` or a blocking poll loop
inside `future T` would stop executor progress. Exposing pipe values without an abandonment
authority would allow ordinary cancellation or destruction to leave a live child or zombie.
Periodic probes and feature-specific sleeps are also excluded because they add latency and create
a second scheduling policy outside the reactor contract.

## Public Shape

`Command` remains the sole owner of an unlaunched request. Process-I/O selection is explicit at the
spawn boundary rather than encoded through unrelated command mutations. The public shape
is:
...
One launch authority supports closed and streaming execution. One process owner always reaches
explicit observation or abandonment. Pipe endpoints satisfy ordinary I/O interfaces and generic
transfer. Nonwaiting observation cannot reap twice, and termination retains the exact cleanup
obligation. Generated and host reactors implement the same process-interest contract without a
blocking future path, periodic process polling, raw-PID ownership assumption, duplicate session,
