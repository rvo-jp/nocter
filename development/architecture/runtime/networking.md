...
This document defines the current cross-responsibility contract for network I/O. Implemented
availability remains defined by the exact checked declarations in `std/net/index.nct` and the
behavior guide assigned by `std/README.md`.
...
uses `address:port` for IPv4 and `[address]:port` for IPv6. The numeric address value grammar does
not accept host names, service names, or IPv6 zone identifiers.
...
IPv6 listeners are IPv6-only. Code requiring both families creates one listener per
...
One package-private datagram policy consumes closed target attempt results. Immediate setup retries
only interruption. A synchronous transfer waits through the blocking descriptor wait and one fixed
deadline. An asynchronous transfer uses descriptor readiness, retries the same immediate attempt,
and never invokes the blocking wait adapter. The shared reducer is the sole owner of ready,
interrupted, would-block, failure, malformed-result, source-address, and truncation meanings.

Both transfer surfaces hold one exclusive descriptor borrow for the complete operation, so close,
timeout mutation, and competing transfer cannot race a suspended wait. Cancelling a suspended send
or receive removes its wait registration without closing or duplicating the borrowed socket. The
socket remains usable after cancellation.

Generic syscall primitives remain conservatively `blocking`; runtime syscall numbers cannot prove
whether an invocation waits. Closed datagram roles instead fix each immediate operation's source
contract, signature, target support, invocation details, and result normalization. The target
adapter owns native socket-address and message records, while target-independent standard source
owns retry, deadline, and public-error policy.
