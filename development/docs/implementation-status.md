...
| LSP | Basic LSP supports initialize, shutdown, full document sync, diagnostics, semantic tokens, hover, definition, references, document symbols, and position-aware basic completion using compiler facts. Block-scope `use` visibility is reflected in completion, references, and semantic tokens. Enum pattern variants are backed by typecheck facts for semantic tokens, hover, definition, and references, and pattern-context completion offers enum variants after `Enum.`. |
