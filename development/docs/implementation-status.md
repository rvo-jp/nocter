...
at the same boundary. Non-empty fixed array literal local bindings, fixed array
local copy bindings, matching fixed array call-result bindings, fixed array
value parameters, direct literal/local/call-result fixed array returns,
whole-local fixed array assignment, constant and variable index reads, and
constant index writes are buildable for the current scalar/view element subset,
with numeric constant index compound assignment for `i32`, `u8`, and `usize`.
...
| Slices, fixed arrays, and vectors | Scalar, `&str`, and current copy-aggregate slice indexing and assignment paths are supported, including numeric scalar slice element compound assignment. Non-empty fixed array literal local bindings, local copy bindings, matching call-result bindings, value parameters, direct literal/local/call-result returns, whole-local assignment, constant and variable index reads for `i32`, `u8`, `usize`, `bool`, and `&str`, constant index writes for the same element subset, and numeric constant index compound assignment for `i32`, `u8`, and `usize` build and run. `Vec<T>` supports scalar, `&str`, and promoted copy-aggregate element storage paths. |
...
- broader fixed-array behavior: variable index writes, zero-sized local
  arrays, move-only element arrays, and general array runtime behavior
