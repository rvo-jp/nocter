...
| Errors and optionals | `T!`, `T?`, postfix `?`, postfix `!`, `catch`, `none`, and `otherwise` are parsed and checked. `catch` blocks that can fall through are rejected by typechecking. Scalar/view and supported aggregate paths build and run when `catch` failure blocks use the current direct-return/effect-only terminal subset and `otherwise` is applied directly to optional-returning calls in supported binding or return positions, plus fixed-array whole-local and aggregate-field assignment positions. Broader catch/otherwise control-flow endings and general `otherwise` expression positions reject before IR lowering. Nested fallible/optional return shapes remain limited. |
