...
includes `if is Enum.variant(binding)` statements and value expressions over
existing values for `i32`, `u8`, `usize`, `bool`, and `&str` payloads. Payload
`match` binding, temporary-pattern control, aggregate/slice/non-copy payload
binding, and active payload drop cleanup remain separate work.
...
- payload-carrying enum `match` binding, aggregate/slice/non-copy payload
  binding, temporary-pattern control, and active payload drop cleanup
