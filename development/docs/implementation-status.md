...
current runtime-supported payload subset, including direct-drop aggregate
payload fields in active-cleanup paths. Payload pattern control-flow lowering
...
`bool`, `&str`, slice views, copy aggregate payloads, and owned direct-drop
aggregate payloads. Move bindings transfer the payload bytes into a branch-local
owner and conditionally suppress cleanup of the source enum.
Scope-end cleanup, parameter cleanup, discarded call results, call-result
bindings, and whole-local replacement now drop only the active direct-drop
aggregate payload fields, using reverse aggregate field order for multi-field
payloads. Supported pattern target temporaries are materialized into hidden
aggregate slots and cleaned up before branch returns or after normal
control-flow completion. Broader pattern target expressions and move-only
aggregate payload bindings without direct drop glue remain separate work.
...
| Enum values | Payloadless enum tag equality, `if is`, and `match` are runtime-shipped. `match` fallback uses `_ { ... }`; enum payload discard uses `Enum.variant(_)` in `match` and `if is`. Payload-carrying enum ABI layout is implemented as a tag byte plus aligned payload union; construction, local slots, returns, value arguments including direct constructors, tag-only `if is Enum.variant(_)` statements, tag-only `match` statements/value expressions, and scalar, string/slice view, copy aggregate, and owned direct-drop aggregate payload bindings in `if is Enum.variant(binding)` statements/value expressions and `match` statement/value-expression arms over existing enum bindings, parameters, and supported call/constructor/move-local pattern targets build/run for the current runtime-supported payload subset, including wildcard-only, nonexhaustive no-wildcard, and exhaustive no-wildcard statement forms. Active payload cleanup build/runs for enum values whose droppable variants have direct-drop aggregate payload fields, covering scope end, parameters, discarded call results, call-result bindings, whole-local replacement, and supported pattern target temporaries. A move binding conditionally transfers cleanup ownership from the source enum to the branch-local payload binding. Multi-field payload cleanup drops active direct-drop fields in reverse aggregate field order. Broader payload enum pattern target expressions and move-only aggregate payload bindings without direct drop glue are not runtime-shipped; unsupported binding types receive a source-backed diagnostic on the binding rather than a generic control-expression rejection. |
...
- payload-carrying enum move-only aggregate payload bindings without direct drop
