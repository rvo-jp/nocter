...
current copy/no-drop payload subset. Payload pattern control-flow lowering,
payload binding, and active payload drop cleanup remain separate work.
...
- payload-carrying enum payload binding, temporary-pattern control, and active
  payload drop cleanup
