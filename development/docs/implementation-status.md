...
move-local pattern targets for `i32`, `u8`, `usize`, `bool`, `&str`, slice
views, copy aggregate payloads, and owned recursively droppable aggregate
payloads. Move bindings transfer the payload bytes into a
...
control-flow completion. This includes direct fallible-call success values
extracted by postfix `?`, postfix `!`, or `catch`. Struct cleanup invokes the
outer destructor first and then recursively drops owned fields in reverse
