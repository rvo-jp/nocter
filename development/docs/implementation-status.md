...
and owned recursively droppable aggregate payloads. Move bindings transfer the
...
for multi-field payloads. Supported pattern target temporaries are materialized into hidden
...
declaration order. Broader pattern target expressions and move-only aggregate
payload bindings with unsupported drop trees remain separate work.
...
- payload-carrying enum move-only aggregate payload bindings with unsupported
