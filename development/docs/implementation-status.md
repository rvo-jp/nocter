...
`u8`, `usize`, `bool`, `&str`, and copy aggregate payloads. Scope-end cleanup,
parameter cleanup, discarded call results, call-result bindings, and
whole-local replacement now drop only the active single direct-drop aggregate
payload. Temporary-pattern control, slice payload binding, non-copy aggregate
...
- payload-carrying enum slice payload binding, non-copy aggregate payload binding,
