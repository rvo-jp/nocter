...
statements are in scalar/view runtime positions. Non-copy aggregate slice
indexing and slice index assignment now reject at the same buildability boundary
instead of leaking into IR diagnostics. IR type normalization and drop glue
resolution also handle source-qualified imported type names when lowering
...
- ordinary dynamic `error` success-return helper ABI outside direct/static
  failure-payload construction
