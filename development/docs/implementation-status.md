...
reads/writes are buildable for the current scalar/view element subset.
...
- broader fixed-array behavior: parameters, returns, variable index
  reads/writes, compound index assignment, zero-sized local arrays, move-only
  element arrays, and general array runtime behavior
