...
supported call/constructor/move-local pattern targets for `i32`, `u8`, `usize`,
`bool`, `&str`, slice views, copy aggregate payloads, and owned recursively
droppable aggregate payloads. Move bindings transfer the payload bytes into a
...
control-flow completion. Struct cleanup invokes the outer destructor first and
then recursively drops owned fields in reverse declaration order. Broader
pattern target expressions and move-only aggregate payload bindings with
unsupported drop trees remain separate work.
