...
reads are buildable for the current scalar/view element subset. Unreachable body
tails after proven terminal statements are ignored by return/reachability
...
- broader fixed-array behavior: parameters, returns, element mutation/index
  assignment, variable index reads, zero-sized local arrays, move-only element
