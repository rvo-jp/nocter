...
| Struct aggregates | Struct literals, fields including optional-call `otherwise` member roots, copies, explicit moves, direct and indirect aggregate parameters and returns, call-result slots, selected assignments, numeric field compound assignment, replacement drops, and cleanup paths are implemented for the current subset. Struct construction records recursive per-field runtime obligations across locals, nested struct and fixed-array fields, staged replacement, returns, and owned arguments. A field becomes live only after its initializer completes; failure unwinds completed fields in reverse source initialization order without invoking direct drop glue on an incomplete struct. |
