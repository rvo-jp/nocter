...
current runtime-supported payload subset, including single direct-drop aggregate
payload variants in active-cleanup paths. Payload pattern control-flow lowering
now includes `if is Enum.variant(binding)` statements/value expressions and
`match` statement/value-expression arm bindings over existing values for `i32`,
`u8`, `usize`, `bool`, and `&str` payloads. Scope-end cleanup, parameter
cleanup, discarded call results, call-result bindings, and whole-local
replacement now drop only the active single direct-drop aggregate payload.
Temporary-pattern control, aggregate/slice/non-copy payload binding, and
multi-field droppable payload cleanup remain separate work.
...
| Enum values | Payloadless enum tag equality, `if is`, and `match` are runtime-shipped. `match` fallback uses `_ { ... }`; enum payload discard uses `Enum.variant(_)` in `match` and `if is`. Payload-carrying enum ABI layout is implemented as a tag byte plus aligned payload union; construction, local slots, returns, value arguments including direct constructors, tag-only `if is Enum.variant(_)` statements, tag-only `match` statements/value expressions, and scalar/view payload bindings in `if is Enum.variant(binding)` statements/value expressions and `match` statement/value-expression arms over existing enum bindings and parameters build/run for the current runtime-supported payload subset, including wildcard-only, nonexhaustive no-wildcard, and exhaustive no-wildcard statement forms. Active payload cleanup build/runs for enum values whose droppable variants each have exactly one direct-drop aggregate payload, covering scope end, parameters, discarded call results, call-result bindings, and whole-local replacement. Payload enum temporary-pattern control, aggregate/slice/non-copy payload binding, and multi-field droppable payload cleanup are still not runtime-shipped. |
...
- payload-carrying enum aggregate/slice/non-copy payload binding,
  temporary-pattern control, and multi-field droppable payload cleanup
