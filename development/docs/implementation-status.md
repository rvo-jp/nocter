...
| Struct aggregates | Struct literals, fields including optional-call `otherwise` member roots, copies, explicit moves, direct and indirect aggregate parameters and returns, call-result slots, selected assignments, numeric field compound assignment, replacement drops, and cleanup paths are implemented for the current subset. Fully initialized move-only fixed-array fields support staged replacement and recursive cleanup through local and read-write-borrowed owners, including nested struct replacement. |
| Enum values | Payloadless enum tag equality, `if is`, and `match` are runtime-shipped. `match` fallback uses `_ { ... }`; enum payload discard uses `Enum.variant(_)` in `match` and `if is`. Payload-carrying enum ABI layout is implemented as a tag byte plus aligned payload union; construction, local slots, returns, value arguments including direct constructors, tag-only `if is Enum.variant(_)` statements, tag-only `match` statements/value expressions, and scalar, string/slice view, copy aggregate, owned recursively droppable aggregate, and supported move-only fixed-array payload bindings in `if is Enum.variant(binding)` statements/value expressions and `match` statement/value-expression arms over existing enum bindings, parameters, and supported owned call-expression, constructor, and move-local pattern targets build/run for the current runtime-supported payload subset, including wildcard-only, nonexhaustive no-wildcard, and exhaustive no-wildcard statement forms. Owned call expressions include direct fallible calls handled by `?`, `!`, or `catch` and direct optional calls handled by `otherwise`. Active payload cleanup build/runs for enum values whose droppable variants have supported recursive aggregate drop trees, covering scope end, parameters, discarded call results, call-result bindings, whole-local replacement, and supported pattern target temporaries. A move binding conditionally transfers cleanup ownership from the source enum to the branch-local payload binding. Multi-field payload cleanup drops active fields in reverse aggregate field order; fixed-array payload elements drop in reverse index order. Direct and indirect enum layouts and concrete generic enum payload substitution use the same ownership rules. Broader payload enum pattern target expressions and move-only payload bindings with unsupported recursive drop trees are not runtime-shipped; unsupported binding types receive a source-backed diagnostic on the binding rather than a generic control-expression rejection. |
| Ownership and drop | The typechecker rejects common use-after-move, double move/drop, invalid drop, borrow conflicts, escaping local borrows, and implicit copies of move-only owned values such as non-copy structs, non-copy concrete generic `copy struct` instantiations, fixed arrays with non-copy elements, optionals, fallibles, and payload-carrying enums. Copy payload pattern bindings copy into the branch; move-only payload bindings require an owned target, so an existing local must be matched through explicit `move` while call and constructor targets already own their temporary. Member targets remain unavailable for move-only bindings until field moves are supported. Typechecker facts record the payload binding mode for lowering. Borrow last-use tracking stops at typed terminal control statements, including terminal `if`/`if is` and exhaustive `match`, so unreachable later uses do not extend active borrows. Lowering invokes a struct's destructor and then recursively drops supported owned struct fields in reverse declaration order; enum cleanup applies the same tree only to active payloads, including reverse-index cleanup for supported fixed arrays. Fully initialized local fixed arrays of supported droppable structs clean elements in reverse index order, transfer cleanup ownership through explicit local moves, restore cleanup after valid reinitialization, and retain recursive cleanup when stored in or used to replace a struct field or payload enum variant. Propagation cleanup reserves the original four-word error payload before lowering recursive drop temporaries. Explicit extraction moves and initializer exits that require partial field state remain rejected. Buildability rejects unsupported explicit aggregate moves in control-flow conditions and outer aggregate moves/drops inside non-terminal control-flow branches/bodies unless that path is one of the current immediate-function-exit forms. |
...
- ordinary input-dependent `error` success-return helper ABI outside direct
  ...
  wrappers
- broader fixed-array behavior: per-element initialization state, per-field
  initialization state for exiting initializers, field extraction moves, and
  general non-copy array indexing/mutation
