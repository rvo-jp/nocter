...
current copy/no-drop payload subset. Payload pattern control-flow lowering now
includes `if is Enum.variant(binding)` statements/value expressions and `match`
statement arm bindings over existing values for `i32`, `u8`, `usize`, `bool`,
and `&str` payloads. Payload `match` value expressions, temporary-pattern
control, aggregate/slice/non-copy payload binding, and active payload drop
...
| Enum values | Payloadless enum tag equality, `if is`, and `match` are runtime-shipped. `match` fallback uses `_ { ... }`; enum payload discard uses `Enum.variant(_)` in `match` and `if is`. Payload-carrying enum ABI layout is implemented as a tag byte plus aligned payload union; construction, local slots, returns, value arguments, tag-only `if is Enum.variant(_)` statements, tag-only `match` statements, and scalar/view payload bindings in `if is Enum.variant(binding)` statements/value expressions and `match` statement arms over existing enum bindings and parameters build/run for copy/no-drop payloads, including wildcard-only, nonexhaustive no-wildcard, and exhaustive no-wildcard statement forms. Payload `match` value expressions, payload enum temporary-pattern control, aggregate/slice/non-copy payload binding, and active payload drop cleanup are still not runtime-shipped. |
...
- payload-carrying enum `match` value expressions, aggregate/slice/non-copy
  payload binding, temporary-pattern control, and active payload drop cleanup
