...
| Ownership, borrowing, move, drop | ship | Frontend rejects use-after-move, double move/drop, invalid drop, borrow conflicts, escaping local borrows, and implicit non-copy aggregate copies for frontend-shipped forms. Runtime lowering inserts drops for the documented subset. Explicit outer aggregate drops inside non-terminal control-flow branches/bodies reject unless the path immediately exits the function. Other unsupported drop/control-flow joins reject before IR or backend emission. |
