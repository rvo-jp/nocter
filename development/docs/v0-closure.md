...
| Arrays, pointers, and general views | runtime ship narrow | Type syntax and borrow/view facts are checked. Runtime supports the current scalar, `&str`, and copy-aggregate slice element paths, non-empty local fixed array literals, local copy bindings, matching call-result bindings, value parameters, direct literal/local/call-result returns, whole-local assignment, constant and variable index reads for `i32`, `u8`, `usize`, `bool`, and `&str`, constant index writes for the same element subset, numeric constant index compound assignment for `i32`, `u8`, and `usize`, public `std/ptr.addr`/`from_ref`/`from_ref_mut` pointer address conversions, and trusted `pub(nocter)` raw-storage pointer primitives. Variable index writes, move-only element arrays, pointer dereference, broad storage, and general iteration reject or defer. |
