...
| Aggregates and ABI | runtime ship narrow | Non-generic structs and fully concrete generic structs use correct direct/indirect ABI classification for supported parameters, arguments, returns, field loads/stores, copies, explicit moves, call-result slots, replacement assignments, and cleanup. Fixed arrays use ABI layout/classification and support the current local-literal/local-copy/call-result-binding/value-parameter/direct-literal-return/local-or-call-result-return/whole-local-assignment/constant-or-variable-index-read/simple-index-write runtime subset, including zero-length fixed arrays where those forms otherwise match, plus numeric-constant-index-compound assignment. Payload enum runtime and broader aggregate expressions remain outside runtime closure unless promoted. |
