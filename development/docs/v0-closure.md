...
| Fallible values | runtime ship narrow | `T!`, built-in `error`, direct dynamic `Error.new(...)` construction, static `error` helper wrappers, postfix `?`, postfix `!`, `catch`, and cleanup during propagation build and run for scalar/view/void and supported aggregate paths. `error` is copyable and borrow-like for return provenance, and `error` parameters are runtime-shipped in the supported call subset. Ordinary dynamic `error` success-return helper calls are not a runtime ABI surface and reject before IR lowering. Fallible success types that are `error` directly or through optional success layers reject during typechecking. `catch` blocks that can fall through reject during typechecking. Runtime-shipped `catch` failure blocks use the current direct-return/effect-only terminal subset; broader terminal control-flow endings reject before IR lowering. Nested fallible/optional return layers remain limited. |
