...
| Fallible values | runtime ship narrow | `T!`, built-in `error`, static and dynamic error construction in the supported call subset, postfix `?`, postfix `!`, `catch`, and cleanup during propagation build and run for scalar/view/void and supported aggregate paths. Fallible success types that are `error` directly or through optional success layers reject during typechecking. `catch` blocks that can fall through reject during typechecking. Runtime-shipped `catch` failure blocks use the current direct-return/effect-only terminal subset; broader terminal control-flow endings reject before IR lowering. Nested fallible/optional return layers remain limited. |
