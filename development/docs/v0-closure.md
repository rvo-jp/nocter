...
| Blocks and control expressions | runtime ship narrow | Body final-expression values, terminal and supported non-terminal `if`, value-producing `if`, payloadless enum `if is` and `match` using `_ { ... }` fallback arms where needed, tag-only payload-carrying enum `if is Enum.variant(_)` statements and tag-only payload-carrying enum `match` statements over existing copy/no-drop enum values, including wildcard-only, nonexhaustive no-wildcard, and exhaustive no-wildcard statement forms, wildcard-only payloadless enum `match`, value-producing supported `match`, supported leading statements inside value-producing branch blocks, `while`, `loop`, `i32`/`usize` range `for`, `break`, `continue`, `return`, and `never` cleanup build and run in the documented subset. Broader effects and joins reject. |
