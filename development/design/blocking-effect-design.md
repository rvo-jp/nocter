...
Phase 2 has moved allocator page mapping, page release, descriptor close, hash-seed filling, and
timeout waiting to the closed `MemoryMap`, `MemoryUnmap`, `DescriptorClose`, `EntropySeedFill`, and
`TimeoutWait` roles. Their Darwin numbers, fixed mapping or timeout layouts, and result
normalization now have one target-backend owner; standard source no longer uses runtime syscall
values to identify them. This migration is intentionally separate from the remaining network
lifecycle work: a narrower primitive role cannot make a synchronous release barrier safe inside
future cancellation.
