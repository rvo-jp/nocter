...
transport only through `TcpStream` and the canonical asynchronous or explicit blocking byte-I/O
contracts.
...
status and headers and implements both `Reader` and `BlockingReader` for the decoded response
body. The reader
