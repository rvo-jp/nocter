...
signature, target support, Darwin syscall identity, fixed flags, native record layout, and result
normalization. The initial role set covers:

- configured UDP descriptor creation;
- numeric bind;
...
typed operands; the ARM64 backend owns native ABI details. Standard source owns retry, deadline,
and public error policy without knowing syscall numbers or native structures.
