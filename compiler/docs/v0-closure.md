...
| Scalar and view values | runtime ship narrow | `i32`, `usize`, `u8`, `bool`, `void`, `never`, `&str`, string literals, byte literals, slices, supported arithmetic/comparison/bool operations, runtime traps, `.len()`, `.is_empty()`, and supported indexing build and run. Broader scalar/view forms reject before backend emission. |
