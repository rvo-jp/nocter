...
| Fallible values | runtime ship narrow | Entry `T!`, static `error` failures, propagation, forced unwrap, and `catch` work for the scalar/view/void and supported aggregate call-result subset. Nested fallible/optional return types and other fallible payload shapes reject before backend emission; direct call expressions inside static failure payload arguments reject in the buildability preflight. |
| Optional values | runtime ship narrow | Optional scalar/view and supported aggregate success/none returns, force unwrap, `let ... else`, `??` defaults, and direct optional scalar/view call `if let`/`while let` branches build and run in the documented subset. Optional branching over non-call initializers, aggregate payloads, borrowed optional projections, and broader optional storage forms remain frontend-only or rejected by build until promoted. |
| Control flow | runtime ship narrow | Terminal `if`, supported non-terminal `if`, `while`, `loop`, direct optional scalar/view call `if let`/`while let`, payloadless enum `if is`, payloadless enum `match`, scalar/view-producing payloadless enum `?{}`, `i32`/`usize` range `for`, `break`, `continue`, `return`, void or ignored scalar/view/aggregate call expression statements, and `never` cleanup build and run in the documented subset. Other value-producing expression statements, explicit aggregate moves in control-flow conditions, optional control-flow outside the direct-call scalar/view subset, payload-carrying enum pattern control flow, non-`i32`/`usize` range `for`, and broader pattern conditional values are frontend-only or rejected by build until promoted. |
...
| Methods | runtime ship narrow | Inherent method declarations, receiver rules, method resolution, drop members, and associated functions are checked. Same-file and imported associated function calls plus method calls whose implicit borrow receiver is a local binding or aggregate field rooted at a binding build in the documented scalar/view/aggregate call subset. Temporary borrow receivers and broader receiver-place lowering are rejected by the buildability preflight until promoted. |
