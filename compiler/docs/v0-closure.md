...
| Fallible values | runtime ship narrow | Entry `T!`, static `error` failures, propagation, forced unwrap, and `catch` work for the scalar/view/void and supported aggregate call-result subset. Other fallible payload shapes reject before backend emission; direct call expressions inside static failure payload arguments reject in the buildability preflight. |
