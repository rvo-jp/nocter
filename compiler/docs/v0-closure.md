...
| Control flow | runtime ship narrow | Terminal `if`, supported non-terminal `if`, `while`, and `loop`, `break`, `continue`, `return`, effect-only expression statements, and `never` cleanup build and run in the documented subset. Value-producing expression statements, `if let`, `if is`, `while let`, range `for`, full `match`, and pattern conditional `?{}` are frontend-only or rejected by build until promoted. |
