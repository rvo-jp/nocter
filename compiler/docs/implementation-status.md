...
| General `if`, `while`, `loop`, `for`, `match`, `?{}` | yes | yes | partial | partial | partial | Several forms are checkable. Build supports a narrow non-terminal `if` subset before a final return, with branch-local bindings, void call statements, nested non-terminal `if`, and branch-local scope-end drops. Loops, range `for`, `match`, and broader `if` forms remain unsupported by backend lowering. |
...
- general `if` forms beyond the supported terminal and branch-local non-terminal subsets, `while`, `loop`, range `for`, `match`, and pattern conditional `?{}`
...
- optional values
- general aggregate value expressions outside the supported struct-literal, call-result, slot-copy, explicit-move, explicit-drop, whole-binding replacement, and borrow paths; arrays, views, pointers, methods, traits, generics, automatic ownership lowering, and general branch/loop drop cleanup
