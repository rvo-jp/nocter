...
| Standard library API names | yes | yes | partial | partial | partial | `.nocter/std` includes initial prelude, error, memory, owning string, formatting, pointer, OS, and I/O declarations with the initial `File` method surface and private close-on-drop state; the active target overlay provides `std/process`. Many bodies are placeholders or primitive boundaries. |
