...
| Terminal `if` / `else` | yes | yes | yes | partial | yes | Build supports terminal branches that contain supported leading explicit `drop` or void call statements followed by direct return, or contain a nested terminal `if`, for direct entry `i32` or `void`, and non-entry `i32`, `u8`, `usize`, `bool`, `void`, `&str`, u8 slices, or supported aggregate return expressions. Direct aggregate branches with pending drops are staged through branch-local aggregate slots before copying to the direct return location. |
