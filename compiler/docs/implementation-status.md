...
| Terminal `if` / `else` | yes | yes | yes | partial | yes | Build supports terminal branches that directly return, or contain a nested terminal `if`, for direct entry `i32` or `void`, and non-entry `i32`, `u8`, `usize`, `bool`, `void`, `&str`, u8 slices, or aggregate struct literals without pending direct-aggregate drop cleanup. |
