...
| General `if`, `while`, `loop`, `for`, `match`, `?{}` | yes | yes | partial | partial | partial | Several forms are checkable. Build supports narrow non-terminal `if` and `while` subsets before a final return, with branch/body-local bindings, void call statements, nested supported non-terminal control flow, and local scope-end drops. `while` re-evaluates lowerable condition instructions at the loop head. `while let`, `break`, `continue`, range `for`, `match`, and broader control-flow effects remain unsupported by backend lowering. |
...
- non-terminal `if` and `while` statements before a final return, when branches/bodies contain supported local bindings, void call statements, or nested supported non-terminal `if`/`while` statements; local aggregate values with drop glue are dropped at branch/body scope end, and lowerable `while` condition instructions are re-evaluated at the loop head
