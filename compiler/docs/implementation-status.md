...
| Ownership, borrowing, move, drop | yes | partial | partial | partial | partial | Explicit scalar and aggregate borrow expressions are parsed and checked, `&+` requires a writable `var` binding, `move name` parses and checks the v0 binding-name operand shape, and local scalar plus aggregate slot borrow arguments are buildable for normal calls. Reserved aggregate slot `return move name` lowers through the aggregate return copy path. Full semantic checking, dereference, escapes, use-after-move, move-only/copy classification enforcement, moved-value drop suppression, and drop glue are not implemented. |
