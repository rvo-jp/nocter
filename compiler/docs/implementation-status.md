...
| Standard library API names | yes | yes | partial | partial | partial | `.nocter/std` includes initial prelude, error, memory, owning string with private `ptr`/`len`/`capacity` fields, formatting, pointer, OS, I/O declarations with the initial `File` method surface and private close-on-drop state, `std/io_impl` raw file-descriptor helpers, and `std/process`; target-dependent primitive declarations use `#target`. Many bodies are placeholders or primitive boundaries. |
