...
| String literals and interpolation | yes | yes | partial | partial | partial | Single-line and multi-line string literals are tokenized, parsed, typed as `&str`, buildable as `&str` call arguments, buildable as direct non-entry `&str` returns, and buildable through annotated `&str` locals plus narrow `&str` normal-call result staging. Interpolation is parsed as an explicit expression and checked as `String!` with limited supported part types. The initial `std/string` and `std/fmt` API boundary exists, and aggregate call-result/borrow prerequisites are partially buildable, but bare interpolation lowering is still disabled. |
| Immutable `let` bindings | yes | yes | yes | partial | yes | Build supports lowerable `i32`, annotated `u8`, annotated `usize`, `bool`, annotated `&str`, annotated slice initializers, narrow aggregate call result bindings used by a same-function aggregate return, and ABI-indirect struct-literal aggregate locals with word-sized fields. |
| `var` and reassignment | yes | yes | partial | partial | partial | Build supports stack-backed scalar/view `var` initializers and simple whole-binding `=` assignment in the current straight-line leading-statement subset. ABI-indirect aggregate call-result and struct-literal `var` slots are buildable for aggregate borrow arguments and same-function return-by-name. Aggregate reassignment, field/index assignment, compound assignment, reinitialization, and drop-aware replacement are not buildable. |
...
| General non-tail calls | yes | yes | yes | partial | partial | Build supports non-generic scalar/view normal calls in selected expression positions, including `&str` results staged into annotated `&str` locals or `&str` call arguments. ABI-indirect aggregate normal and propagated fallible calls can be staged into reserved aggregate slots for narrow `let`/`var` binding paths, and aggregate slot borrows can be passed as `&T`/`&+T` arguments. Unsupported shapes still report IR lowering diagnostics. |
...
| Fallible entry success/failure | yes | yes | partial | partial | partial | Build supports success, static `error` constructor failure returns, propagated/caught failures through the current scalar/view/void call subset, and propagated ABI-indirect aggregate call-result staging, reporting `code: message` on stderr. |
| Optional values | yes | yes | yes | no | no | Check examples cover optionals; backend and runtime layout are not implemented. |
| Structs and enums | yes | yes | partial | partial | partial | Type checking exists for several cases. Build supports ABI-indirect struct return signatures, non-entry struct literal returns with word-sized fields, ABI-indirect struct-literal local slots, direct aggregate return calls, narrow aggregate call result slot returns, and aggregate slot borrow arguments; general and direct aggregate values remain unsupported. |
...
| Ownership, borrowing, move, drop | yes | partial | partial | partial | partial | Explicit scalar and aggregate borrow expressions are parsed and checked, `&+` requires a writable `var` binding, and local scalar plus ABI-indirect aggregate slot borrow arguments are buildable for normal calls. Full semantic checking, dereference, escapes, source-level move, and drop glue are not implemented. |
