...
| Scalars and strings | `i32`, `usize`, `u8`, `bool`, `void`, `never`, `&str`, string literals, byte literals, checked `i32`/`usize`/`u8` arithmetic and shifts, comparisons, bool operators, and runtime trap checks are implemented in the buildable subset. |
