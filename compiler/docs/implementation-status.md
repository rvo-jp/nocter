...
| General non-tail calls | yes | yes | yes | partial | partial | Build supports non-generic scalar/view normal calls in selected expression positions, including `&str` results staged into annotated `&str` locals or `&str` call arguments. ABI-indirect aggregate normal calls can be staged into a reserved aggregate slot only for the narrow `let value = make(); return value` path. Unsupported shapes still report IR lowering diagnostics. |
