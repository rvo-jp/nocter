...
- supported non-terminal `if`/`while` branch/body-local assignments, outer whole-binding scalar/view local assignments, explicit drops, return cleanup, body scope-end drops, and `while` `break`/`continue` cleanup
