...
- `return move name` from reserved aggregate slots, with straight-line, conditional, and switch ownership-state checking but without drop lowering
...
- broader loop/reachability owned aggregate move/drop state joins and drop lowering beyond reserved-slot `return move name`
