...
2. Add the next runtime prerequisites for allocation-backed `String`: owned aggregate move/drop state tracking beyond reserved-slot `return move name`, then target-backed allocation and mutation in `std/mem`, `std/string`, and `std/fmt`. IR already recognizes ABI-indirect and direct aggregate return signatures plus aggregate borrow parameters, has low-level aggregate slot/call/fallible-call/direct-call/fallible-direct-call/`usize` field-store/copy primitives, and lowers direct aggregate struct literal returns, direct aggregate normal/fallible call-result slots, direct and indirect aggregate by-value arguments inside the 8-word register window, non-entry indirect aggregate struct literal returns with 8-byte integer fields or `std/ptr.from_addr` pointer fields, aggregate struct-literal local slots for those same field shapes, narrow normal/fallible indirect aggregate call result bindings in reserved `let`/`var` slots, supported aggregate slot assignment including copy struct slot-to-slot assignment, aggregate slot borrow arguments, return-by-name from those slots, and reserved-slot `return move name`; stack-passed arguments, general pointer expressions, source-level aggregate moves beyond reserved-slot return, and aggregate locals outside the supported call-result/struct-literal paths are not buildable yet. Loaded imported scalar calls, scalar parameters/call arguments, local scalar borrow arguments for `&T` and `&+T`, `&str` call arguments, direct `&str` returns, annotated `&str` locals, narrow `&str` normal-call result staging, scalar/view stack-backed `var` plus simple `=` assignment, and ordinary fallible propagation for the current scalar/view/void call subset are already buildable.
...
- Avoid broad `if`, `while`, `loop`, `match`, non-scalar `var`, compound reassignment, and aggregate forms outside the current register-only slot/call-result subset until backend storage and ABI rules are ready.
