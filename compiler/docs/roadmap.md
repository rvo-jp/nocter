...
2. Add only the remaining backend prerequisites needed by that explicit path: aggregate reassignment for mutable owned values, direct aggregate values where the std surface still requires them, and owned aggregate return/move body lowering. IR already recognizes ABI-indirect aggregate return signatures and aggregate borrow parameters, has low-level aggregate slot/call/fallible-call/`usize` field-store/copy primitives, and lowers direct aggregate return calls, non-entry struct literal returns with `usize` fields or `std/ptr.from_addr` pointer fields, ABI-indirect struct-literal local slots for those same field shapes, narrow normal/fallible aggregate call result bindings in reserved `let`/`var` slots, aggregate slot borrow arguments, and return-by-name from those slots; aggregate by-value arguments, direct aggregate values of 16 bytes or smaller, general pointer expressions, assignment/reassignment, source-level aggregate moves, and aggregate locals outside the supported call-result/struct-literal paths are not buildable yet. Loaded imported scalar calls, scalar parameters/call arguments, local scalar borrow arguments for `&T` and `&+T`, `&str` call arguments, direct `&str` returns, annotated `&str` locals, narrow `&str` normal-call result staging, scalar/view stack-backed `var` plus simple `=` assignment, and ordinary fallible propagation for the current scalar/view/void call subset are already buildable.
