...
- Avoid broad `if`/`while`, `while let`, `loop`, `match`, non-scalar `var`, compound reassignment, and aggregate forms outside the current supported slot/call-result subset until backend storage and ABI rules are ready.
