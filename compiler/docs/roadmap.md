...
3. Keep imported calls, aggregate values, nested tail-call arguments, and broader control-flow disabled until their lowering rules are designed.

## Near-Term Constraints

- Keep nested tail-call argument lowering conservative until tail calls can consume staged child call results.
- Keep bool calls inside short-circuit value expressions disabled until lowering can preserve short-circuit evaluation and materialize the expression result.
