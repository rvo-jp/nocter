...
2. Add only the remaining backend prerequisites needed by that explicit path: aggregate storage outside the supported call-result/struct-literal slot paths and owned aggregate move/drop state tracking. IR already recognizes ABI-indirect and direct aggregate return signatures plus aggregate borrow parameters, has low-level aggregate slot/call/fallible-call/direct-call/fallible-direct-call/`usize` field-store/copy primitives, and lowers direct aggregate struct literal returns, direct aggregate normal/fallible call-result slots, non-entry indirect aggregate struct literal returns with 8-byte integer fields or `std/ptr.from_addr` pointer fields, aggregate struct-literal local slots for those same field shapes, narrow normal/fallible indirect aggregate call result bindings in reserved `let`/`var` slots, supported aggregate slot assignment, aggregate slot borrow arguments, return-by-name from those slots, and reserved-slot `return move name`; aggregate by-value arguments, general pointer expressions, source-level aggregate moves beyond reserved-slot return, and aggregate locals outside the supported call-result/struct-literal paths are not buildable yet. Loaded imported scalar calls, scalar parameters/call arguments, local scalar borrow arguments for `&T` and `&+T`, `&str` call arguments, direct `&str` returns, annotated `&str` locals, narrow `&str` normal-call result staging, scalar/view stack-backed `var` plus simple `=` assignment, and ordinary fallible propagation for the current scalar/view/void call subset are already buildable.
