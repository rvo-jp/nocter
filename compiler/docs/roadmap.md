...
2. Add only the remaining backend prerequisites needed by that lowering: `usize` parameter/call-argument support where the explicit APIs require it, aggregate `String` storage, mutable local mutation through `&+String`, and fallible propagation from append calls. Loaded imported scalar calls are already buildable, and `usize` locals/returns/comparisons now have a backend foundation.
