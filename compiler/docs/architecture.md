...
- non-terminal `if` and `while` statements before a final return, when branches/bodies contain supported local bindings, branch/body-local assignments, branch/body-local explicit aggregate drops, void call statements, supported `while` `break`/`continue`, or nested supported non-terminal `if`/`while` statements; branch/body-local aggregate values with drop glue are dropped at scope end and before supported loop exits, and lowerable `while` condition instructions are re-evaluated at the loop head
