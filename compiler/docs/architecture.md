...
- `.len()` and `.is_empty()` on `&str`, string literals, `&[u8]`, `&+[u8]`, and supported call results in lowerable `usize` or `bool` positions; byte indexing into those values in lowerable `u8` positions
