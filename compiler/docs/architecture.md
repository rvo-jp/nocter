...
- aggregate struct-literal local slots, including explicit aggregate field moves, explicit aggregate move bindings, direct aggregate call-result local slots, narrow indirect aggregate call-result local slots, fallible direct aggregate call-result local slots, and `&T`/`&+T` borrow arguments from those slots
