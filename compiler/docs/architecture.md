...
- same-file non-generic normal calls returning `i32` in `let` initializers, `i32` additions, `i32` comparison operands, and nested `i32` call arguments
...
The source-level scalar call subset lowers same-file non-generic `i32` calls in `let` initializers, `i32` additions, `i32` comparison operands, nested normal-call arguments, and nested tail-call arguments, evaluating staged calls left to right into distinct temporary locals.
