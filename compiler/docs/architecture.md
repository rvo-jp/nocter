...
IR function signatures preserve ABI-indirect aggregate returns as `Type::Aggregate { layout }` and aggregate borrow parameters as one-word `Type::Borrow { inner: Aggregate { .. } }`, so lowering can distinguish owned aggregate return and borrow shapes before aggregate value construction is broadly buildable.
IR `CallAggregate` and `CallFallibleAggregate` lower calls that return indirect aggregate values by passing either the current return-storage pointer or a reserved destination slot address in ABI register `x8`; source lowering emits them for direct aggregate return calls and for narrow aggregate call result bindings.
IR `StoreAggregateUsize` writes 8-byte fields either into the current indirect return storage behind `x8` or into a reserved aggregate stack slot; source lowering emits it for non-entry indirect aggregate struct literal returns and ABI-indirect struct-literal local bindings whose stored fields are `usize` or `std/ptr.from_addr` pointer fields.
IR `CopyAggregate` copies 8-byte chunks from a reserved aggregate slot into the current indirect return storage; source lowering emits it for narrow aggregate `let`/`var` binding paths returned by name.
