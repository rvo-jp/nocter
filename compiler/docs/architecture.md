...
IR `CallAggregate` lowers normal calls that return indirect aggregate values by passing either the current return-storage pointer or a reserved destination slot address in ABI register `x8`; source lowering emits it only for direct aggregate return calls.
