...
- `ir/`: optional lower-level compiler representation if direct AST lowering becomes too tangled. IR call instructions carry a backend-independent `CallTarget` so same-file targets and imported targets do not share raw strings. `CallTarget::Imported` is reserved for loaded imported scalar calls and carries the imported declaration source identity plus function name. `ir/lower/reachability.rs` owns same-file call target discovery for reachable lowering. `ir/lower/imported_calls.rs` owns imported call target collection and the current backend boundary diagnostic that rejects imported call targets before same-file call lowering tries to treat them as reachable local functions.
