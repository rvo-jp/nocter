...
`backend/frame.rs` owns the fixed v0 frame layout planner: frame size, saved `x30` offset, scalar spill-slot offsets, stack-backed argument staging offsets, and reserved aggregate stack slots for narrow aggregate result staging.
...
IR `CallAggregate` lowers normal calls that return indirect aggregate values by passing either the current return-storage pointer or a reserved destination slot address in ABI register `x8`; source lowering emits it for direct aggregate return calls and for narrow aggregate call result bindings.
...
IR `CopyAggregate` copies 8-byte chunks from a reserved aggregate slot into the current indirect return storage; source lowering emits it for the narrow `let value = make(); return value` aggregate path.
