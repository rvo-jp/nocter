...
IR `StoreAggregateUsize` writes 8-byte fields either into the current indirect return storage behind `x8` or into a reserved aggregate stack slot; source lowering emits it only for non-entry indirect aggregate struct literal returns whose stored fields are `usize` or `std/ptr.from_addr` pointer fields.
