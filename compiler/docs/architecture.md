...
- `abi/`: Nocter ABI v0 classification, data layout, aggregate layout, call lowering rules, return lowering rules, and drop glue rules. The current helper layer computes primitive, pointer, borrow, `&str`, slice, and struct layouts plus the 16-byte direct/indirect value classification; later lowering should reuse it instead of duplicating field offset arithmetic in IR or backend code.
