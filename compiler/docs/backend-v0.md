...
`&str` equality and inequality are frontend-checkable but are not part of the v0 backend subset yet; reachable `==` or `!=` over `&str` operands is rejected by the buildability preflight before IR lowering.
...
- aggregate struct-literal field stores cover `u8`, `u16`, `u32`, `bool`, `i32`, `usize`/`u64`, pointer-sized fields, and supported nested aggregate fields in the current slot-backed paths
