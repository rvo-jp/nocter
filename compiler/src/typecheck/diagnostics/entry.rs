...
pub(in crate::typecheck) fn missing_entry_function_diagnostic(
    ...
    diagnostic.help = Some(format!(
        "add `func {entry_name}(): i32! {{ ... }}`, `func {entry_name}(): i32 {{ ... }}`, or `func {entry_name}(): void {{ ... }}`"
    ));
    diagnostic
}
...
pub(in crate::typecheck) fn invalid_entry_function_diagnostic(
    ...
    let mut diagnostic = Diagnostic::error(
        "E0303",
        format!(
            "entry function `{entry_name}` must have no parameters and return `i32!`, `i32`, or `void` in v0"
        ),
    );
    diagnostic.primary_span = sources.span_to_json(function_span).ok().map(Box::new);
    diagnostic.help = Some(format!(
        "use `func {entry_name}(): i32!`, `func {entry_name}(): i32`, or `func {entry_name}(): void`"
    ));
    diagnostic
}
