...
- Current checkpoint: connect std file read to Darwin `read`
  ...
  - emits Darwin syscall `0x02000003` in the backend and normalizes syscall success/failure into the existing fallible call ABI before storing the `usize` success payload
  - covers IR lowering, native backend execution with a non-blocking zero-byte read, distributed std typecheck, and raw-helper privacy
...
- Current checkpoint: lower source loop through while IR
  ...
  - covers non-terminal `loop` before a final return and terminal `loop { return ... }` through IR and native run tests
...
- Current checkpoint: validate backend call return shapes
  ...
  - rejects scalar/view/void and direct/indirect aggregate return shape drift, including hand-built IR that previously encoded aggregate-return callees as `void`
- Current checkpoint: expose IR return ABI passing to backend
  - moves success-return `ReturnPassing` mapping from lowering-private type helpers onto normalized IR `Type`
  - reuses that mapping from aggregate and scalar/view call-result lowering so validation does not duplicate ABI rules
  - validates backend fallible success payload emission against the same return ABI mapping and rejects direct payloads wider than the supported two payload registers
...
- Current checkpoint: index success return ABI passing
  ...
  - validates aggregate normal/fallible call-result lowering against the indexed callee success-return passing before backend codegen
...
- Current checkpoint: normalize alias value ABI in IR lowering
  ...
  - shares resolved type normalization between entry signature validation/lowering, function signature indexing, function return/parameter lowering, annotated scalar/view local bindings, and explicit scalar conversion targets so aliases, including aliases to supported borrow/view/fallible forms, do not fall back to raw spelling checks there
...
- Current checkpoint: tighten supported non-terminal control flow
  - lowers branch/body-local assignments and outer whole-binding scalar/view local assignments in the narrow non-terminal `if`/`while`/`loop` subset while rejecting outer aggregate effects there except outer aggregate whole-binding assignment, direct `target = move source` assignment from an outer aggregate source, explicit drop, or direct `move name` binding followed by a function-exiting suffix
  ...
  - treats supported `never` expression statements as function-exiting suffixes and terminal statements in the narrow non-terminal subset
  - inserts pending aggregate scope-end drops before supported `never` exits, including `return abort()`, terminal-if `abort()` leaves, non-terminal branch/body `abort()` leaves, and catch-handler `never` exits
  ...
  - lowers supported return statements and `never` expression statements inside the narrow non-terminal subset through shared lowerers, while keeping broader ownership joins and condition-sensitive effects disabled until their lowering rules are designed
...
- Current checkpoint: render CLI diagnostics with source snippets
  ...
  - passes the loaded `SourceMap` through build/backend IR lowering so E800x lowering diagnostics can fill missing primary spans from the relevant declaration, statement, or expression
...
- Current checkpoint: implement borrow argument lowering
  ...
  - rejects direct backend `TailCall` emission with borrow arguments so hand-built IR cannot branch with dangling caller-frame addresses
...
The aggregate move/drop backend subset now covers explicit drop glue, straight-line and terminal-if scope-end drops, terminal-if value-return staging, propagation-failure cleanup, supported catch-handler cleanup, supported non-terminal `if`/`while`/`loop` branch/body-local assignments, outer whole-binding scalar/view local assignments, function-exit-only outer aggregate assignment/drop/move-binding effects including direct move assignment from outer aggregate sources, explicit drops, returns, supported `never` expression exits with pending aggregate cleanup, body scope-end drops, `while`/`loop` `break`/`continue` cleanup, unreachable non-terminal scope-drop suppression after terminal nested branches, copy-only aggregate field assignment, and drop-aware whole-binding replacement.
...
2. Add only the remaining backend/runtime prerequisites needed by that explicit path: broader branch/loop scope-end cleanup where required, target-backed allocation, and `std/string`/`std/fmt` mutation behavior.
3. Consider broader non-terminal effects only after ownership joins and cleanup insertion rules are designed.
