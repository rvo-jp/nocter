...
- Current checkpoint: tighten supported non-terminal control flow
  - lowers branch/body-local assignments in the narrow non-terminal `if`/`while` subset while rejecting outer local assignments and outer aggregate moves there except direct `move name` bindings followed by a function-exiting statement
  - lowers outer aggregate explicit drops followed by a function-exiting statement while still rejecting fallthrough and loop-control outer drops
