...
2. Add the next runtime prerequisites for allocation-backed `String`: extend the initial straight-line, conditional, and switch owned aggregate move/drop state checks to broader loop/reachability joins, then target-backed allocation and mutation in `std/mem`, `std/string`, and `std/fmt`.
...
- Current checkpoint: track owned aggregate move/drop state
  ...
  - keeps loop/reachability joins, moved-value drop suppression, replacement drop lowering, and drop glue disabled
