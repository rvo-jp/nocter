...
- Current checkpoint: lower aggregate terminal-if returns
  ...
  - stages direct aggregate struct literal branch returns through branch-local aggregate slots when pending drops must run before returning
  - keeps direct aggregate terminal-if call/local returns with pending drop cleanup unsupported until a general direct-return staging path exists
  - covers indirect/direct IR lowering and CLI build coverage for a direct aggregate terminal-if return with pending drop cleanup
