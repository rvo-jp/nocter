...
- `if`, `switch`, loop exits, `break`, `continue`, `return`, `fail`, and postfix `?` propagation participate in the same state analysis.
...
- Payload-carrying enum equality is not part of the initial design. Use `switch` or `if expr is Pattern`.
...
Adopted: `switch` is the initial control flow form for enum pattern matching.
...
- `switch` is a statement in the initial design.
- Switch arms use `is Pattern { ... }`.
- `else { ... }` is the fallback arm.
- A `switch` may have at most one `else` arm.
- `else` must be the last arm.
- Exhaustiveness checking is deferred in v0, so `switch` without `else` is not treated as a terminating statement yet.
- `switch ... else` is treated as a terminating statement when every explicit arm and the `else` arm terminate.
- Payload names in a pattern are bound only inside that arm block.
- `switch` expressions that return values are deferred.
...
- `if enum_expr is Pattern` uses the same enum pattern syntax as `switch`.
