...
- A copy payload binding copies the payload into its branch-local binding. A
  ...
  for aggregate payloads with runtime-supported recursive drop glue.
...
- Payload-carrying enum `if is` / `match` over call results, direct-call
  ...
  payload subset. Other pattern target expressions and move-only aggregate
  payload bindings with unsupported recursive drop trees still reject before
  build/run.
