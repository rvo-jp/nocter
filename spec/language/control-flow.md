...
Parameters are written as `name: Type`. Mutable parameter bindings are not supported. Parameter binding and ownership rules are specified in [Ownership, Borrowing, and Drop](ownership.md#function-parameters).
...
- A value-producing function body result or explicit `return expression` is checked against the
  ...
  [recursive outcome injection](errors-and-optionals.md#recursive-outcome-injection). This is one
...
- The complete continuation-leader set and lexical source text rules are specified in
  [Lexical Grammar](lexical-grammar.md#statement-separation).
