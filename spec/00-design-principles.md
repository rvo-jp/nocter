...
Nocter deliberately avoids class inheritance. Designs that require users
...
separate from interfaces: an interface describes capability, while stored composition would own a
contained value. It must not grant either side access to the other's private implementation.
